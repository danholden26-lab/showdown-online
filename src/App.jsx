...
const playerCardDeck = [
  ...
  { 
    ...
    chart: [
      ...
      { roll: [19, 19], result: 'triple', text: 'Triple', sticker: 'XBH' },
...
export default function App() {
  ...
  const rollForAtBatResult = async () => {
    ...
    if (newOuts >= 3) {
      ...
      nextInning = game.inning % 1 === 0 ? game.inning + 0.5 : game.inning + 0.5;
        ...
        newBatterIndex = (newBatterIndex + 1) % updatedTeamBatters.length;
    }
  ...
  const advanceToNextAtBat = async () => {
    if (!user || !game || !gameId) return;
  ...
  const updateBases = (currentBases, result) => {
    ...
    if (result.type === 'walk') {
        ...
        let runnersOnBase = [newBases[0], newBases[1], newBases[2]];
        let nextBases = [false, false, false];
        ...
        if (runnersOnBase[2]) { score++; }
        if (runnersOnBase[1]) { score++; }
        if (runnersOnBase[0]) { nextBases[2] = true; }
        ...
        // Runners advance three bases
        let runnersOnBase = [newBases[0], newBases[1], newBases[2]];
        let nextBases = [false, false, false];

        if (runnersOnBase[2]) { score++; }
        if (runnersOnBase[1]) { score++; }
        if (runnersOnBase[0]) { score++; }
  ...
  const inningDisplay = game?.inning % 1 === 0 ? Math.floor(game?.inning) + "st" : Math.floor(game?.inning) + "nd";
    ...
    <div className="min-h-screen bg-gray-900 text-gray-100 font-inter p-4 flex flex-col items-center">
      ...
      {!gameId && (
        <div className="text-center">
          <h1 className="text-4xl font-bold mb-4 flex items-center justify-center gap-2">⚾️ MLB Showdown</h1>
          <p className="mb-8 max-w-lg">Test the core mechanics by playing against yourself!</p>
            ...
            <div className="flex-1 bg-gray-800 p-6 rounded-xl shadow-lg border border-gray-700 text-center mt-8 sm:mt-0">
              ...
              <div className="mb-4">
                {game.atBatPhase === 'secondRoll' && (
                  <div className="flex flex-col items-center mb-4">
                    <p className="text-lg font-medium">
                      Roll: <span className="text-yellow-400 font-bold">{game.lastRoll1}</span>
                    </p>
                    <div className={`p-4 mt-2 rounded-lg text-white font-bold text-lg w-full max-w-xs transition-colors duration-300 ${game.lastAdvantage === 'pitcher' ? 'bg-blue-600' : 'bg-red-600'}`}>
                      {game.lastAdvantage === 'pitcher' ? 'Pitcher\'s Advantage!' : 'Batter\'s Advantage!'}
                    </div>
                  </div>
                )}
                {game.atBatPhase === 'completed' && (
                  <div className="flex flex-col items-center mb-4">
                    <p className="text-lg font-medium">
                      Roll: <span className="text-yellow-400 font-bold">{game.lastRoll2}</span>
                    </p>
                    <div className="p-4 mt-2 rounded-lg bg-green-600 text-white font-bold text-lg w-full max-w-xs transition-colors duration-300">
                      {game.lastResult}
                    </div>
                  </div>
                )}
              ...
              {game.atBatPhase === 'firstRoll' && (
                <button
                  ...
                  className="bg-green-600 hover:bg-green-700 text-white font-bold py-3 px-8 rounded-full transition-colors disabled:opacity-50 disabled:cursor-not-allowed flex items-center justify-center gap-2 w-full"
                >
                  ⚔️
                  Roll for Advantage
                </button>
              ...
              {game.atBatPhase === 'secondRoll' && (
                <button
                  ...
                  disabled={!currentPitcher || !currentBatter}
                  className="bg-red-600 hover:bg-red-700 text-white font-bold py-3 px-8 rounded-full transition-colors disabled:opacity-50 disabled:cursor-not-allowed flex items-center justify-center gap-2 w-full"
                >
                  📊
                  Roll for Result
                </button>
              ...
              {game.atBatPhase === 'completed' && (
                <button
                  ...
                  className="bg-blue-600 hover:bg-blue-700 text-white font-bold py-3 px-8 rounded-full transition-colors flex items-center justify-center gap-2 w-full"
                >
                  ➡️ Next At-Bat
                </button>
              )}
              ...
              <div className="mt-8 space-y-6">
                ...
                {currentPitcher && (
                  <div className={`p-4 rounded-xl shadow-inner border-2 ${currentPitcher.team === 'home' ? 'border-red-600 bg-red-900/20' : 'border-blue-600 bg-blue-900/20'} text-left`}>
                    <h4 className="text-xl font-bold mb-2 flex items-center gap-2">⚾️ {currentPitcher.name} (P)</h4>
                ...
                {currentBatter && (
                  <div className={`p-4 rounded-xl shadow-inner border-2 ${currentBatter.team === 'home' ? 'border-red-600 bg-red-900/20' : 'border-blue-600 bg-blue-900/20'} text-left`}>
                    <h4 className="text-xl font-bold mb-2 flex items-center gap-2">⚾️ {currentBatter.name} (B)</h4>
                    <div className="flex justify-between items-center text-sm mb-2">
